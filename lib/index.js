...
const getSizesToDelete = (file, config) => {
  ...
  const sizesToDelete = imageSizes.map(size => {
    ...
    switch (file.ext) {
      case '.png':
        sizes.push({ suffix: `-${width}-${height}.png` });
        break;
      case '.jpg':
        sizes.push({ suffix: `-${width}-${height}.jpg` });
        break;
      case '.jpeg':
        sizes.push({ suffix: `-${width}-${height}.jpg` });
        break;
...
module.exports = {
  ...
  auth: {
    ...
    s3ForcePathStyle: {
      label:
        's3ForcePathStyle: Whether to force path style URLs for S3 objects',
      type: 'enum',
      values: ['false', 'true']
    },
    signatureVersion: {
